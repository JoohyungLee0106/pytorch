...
net = network.Net_res3D_attention().to(my_cuda_device)
...
for epoch in range(max_epoch):
    ...
    for i, data_tr in enumerate(dataloader_tr, 0):
        ...
        mask_tr_tensor = mask_tr_tensor.cuda(my_cuda_device)
        labels_temp_tr_tensor = labels_temp_tr_tensor.cuda(my_cuda_device)
        optimizer.zero_grad()
        # print("zero grad: " + str(time.time() - bfzg))
# torch.nn.Conv3d : (N, C, D(3rd dim), H, W)

        # bfff=time.time()
        with torch.cuda.device(my_cuda_device_number):
        logits_category_tr, logits_attention_tr = net(inputs_tr_tensor, my_cuda_device)
        ...
        loss_mask_tr = custom_loss.dice_loss(logits_attention_tr, mask_tr_tensor[:,1,:,:,:].double())

        loss_mtl_tr = loss_focal_category_tr+(loss_mask_tr)
        ...
        performance_recorder_trainingset.update_one_epoch_list(probability=probs_temp_tr_tensor.tolist(),
                                   ...
                                   loss_category=loss_focal_category_tr.tolist(), loss_attention=loss_mask_tr.tolist(),
    ...
    for i_val, data_val in enumerate(dataloader_val, 0):
        ...
        mask_val_tensor = mask_val_tensor.cuda(my_cuda_device)
        ...
        loss_mask_val = custom_loss.dice_loss(logits_attention_val, mask_val_tensor[:, 1, :, :, :].double())

        loss_mtl_val = loss_focal_category_val + loss_mask_val
        ...
        performance_recorder_validationset.update_one_epoch_list(probability=probs_temp_val_tensor.tolist(),
                                       ...
                                       loss_attention=loss_mask_val.tolist(),
    ...
    performance_recorder_trainingset.update_tendency_list_by_averaging('accuracy', 'dice_score',
                                                                       'loss_category', 'loss_attention', 'loss_overall')
    performance_recorder_validationset.update_tendency_list_by_averaging('accuracy', 'dice_score',
                                                                       'loss_category', 'loss_attention', 'loss_overall')
    ...
    performance_recorder_trainingset.save_tendency_list_as_txt('accuracy', 'sensitivity', 'specificity', 'dice_score',
                                                                       'loss_category', 'loss_attention', 'loss_overall')
    performance_recorder_validationset.save_tendency_list_as_txt('accuracy', 'sensitivity', 'specificity', 'dice_score',
                                                                       'loss_category', 'loss_attention', 'loss_overall')
